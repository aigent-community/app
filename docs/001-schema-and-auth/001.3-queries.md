...
- API token queries (create/list/revoke/lookup by hash)
...
### 3.4 API Token (`packages/data-ops/src/queries/api-token.ts`)
...
export async function listApiTokensByUserId(userId: string) {
  const db = getDb();
  return db
    .select()
    .from(apiToken)
