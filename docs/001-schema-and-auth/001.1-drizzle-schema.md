...
Drizzle schema, Zod validation, Hono CRUD routes, and credit initialization for the TokSwap marketplace foundation. This sub-doc covers the six new tables, five enums, all relations, and Drizzle config.
...
- Enum types for provider mode, pool status, session status, session end reason, credit type
