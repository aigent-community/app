...
interface Env {
  ...
  ENCRYPTION_KEY: string  // Worker secret, AES-256-GCM for provider API key decryption
}
```

`ENCRYPTION_KEY` -- set via `wrangler secret put ENCRYPTION_KEY` per environment. Used to decrypt provider API keys at runtime. No platform-wide `ANTHROPIC_API_KEY` needed; each provider's key is stored encrypted in DB and decrypted per-session.
